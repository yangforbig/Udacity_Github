    ...
    with open(filename, "r") as f:
        ...
        for line in reader:
            ...
            for field in process_fields:
                if (line[field] == "NULL" and field != "name"):
                    line[field] = None
                elif (line[field] == "NULL" and field == "name"):
                    line[field] = line['rdf-schema#label']
                else:
                    line[field] = line[field].strip()
                    if field == "URI":
                        line[field] = re.search(r'^[^\(]+', line[field]).group()
                    if field == "synonym":
                        if type(line[field]) == 's':
                            line['synonym'] = parse_array(line['synonym'])

            entry = {'label': line['rdf-schema#label'],
                     'uri' : line['URI'],
...
def test():
    ...
    #         "class": "Arachnid"
    #     },
    #     "uri": "http://dbpedia.org/resource/Argiope_",
    #     "label": "Argiope",
